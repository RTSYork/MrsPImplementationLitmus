...
void add_time(long long time1, long long time2, long long time3){
	long i;
	for(i = 0; i < 100000; i++){
		if(response_time[0][i] == 0){
			response_time[0][i] = time1;
			response_time[1][i] = time2;
			response_time[2][i] = time3;
			return;
		}
	}
}
...
static void debug_delay_loop(void)
{
	...
	response_time = malloc(sizeof(long long ) *3);
	for (i = 0; i < 3; i++) {
		response_time[i] = malloc(sizeof(long long) * 100000);
		for(j = 0; j < 100000; j++){
			response_time[i][j] = 0;
		}
	}
	...
	for (i=0; i<100000;i++) {
		...
		if(i % 100 == 0)
			printf("%ld\n", i);
		add_time( (end - start) * 1000000000, (end - start - delay) * 1000000000, 100 * (end - start - delay) / delay * 1000000000);
	}
	...
	for(i = 0; i < 3; i++){
		avg = 0;
		fprintf(f, "task 1 access time:\n");
		for(j = 0; j < 100000; j++){
			if(response_time[i][j] != 0){
				avg += response_time[i][j];
				fprintf(f, "%lld\n", response_time[i][j]);
			}
			else
				break;
		...
		real_avg = avg/j;
		if(i == 0)
			printf("task %ld executes %ld times, exec_avg1: %20.5f\n", i, j, real_avg);
