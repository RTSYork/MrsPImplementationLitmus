...
struct rt_task {
	...
	unsigned int    current_cpu_ceiling;

	volatile int	mrsp_lock_status;
	volatile int	preempt_while_waiting;
	volatile int	is_helping;
	volatile int	is_migrate_manually;
	volatile int	helped;

	int enable_help;
	long long migrated_exec_time;
