...
DEFINE_PER_CPU(mrsp_push_domain_t, mrsp_push_domains);

mrsp_push_domain_t* mrsp_push_doms[NR_CPUS];

spinlock_t mrsp_push_migrate_lock;
struct task_struct*** mrsp_push_tasks;

#define local_pfp		(this_cpu_ptr(&mrsp_push_domains))
#define remote_dom(cpu)		(&per_cpu(mrsp_push_domains, cpu).domain)
#define remote_pfp(cpu)	(&per_cpu(mrsp_push_domains, cpu))
#define task_dom(task)		remote_dom(get_partition(task))
#define task_pfp(task)		remote_pfp(get_partition(task))


#ifdef CONFIG_LITMUS_LOCKING
DEFINE_PER_CPU(uint64_t,mrsp_push_fmlp_timestamp);
#endif

/* we assume the lock is being held */
static void preempt(mrsp_push_domain_t *pfp)
{
...
static void mrsp_push_pfp_release_jobs(rt_domain_t* rt, struct bheap* tasks)
{
	mrsp_push_domain_t *pfp = container_of(rt, mrsp_push_domain_t, domain);
	unsigned long flags;
...
static void mrsp_push_preempt_check(mrsp_push_domain_t *pfp)
{
	if (fp_higher_prio(fp_prio_peek(&pfp->ready_queue), pfp->scheduled))
		preempt(pfp);
}

static void mrsp_push_domain_init(mrsp_push_domain_t* pfp,
			       int cpu)
{
	fp_domain_init(&pfp->domain, NULL, mrsp_push_pfp_release_jobs);
	pfp->cpu      		= cpu;
	pfp->scheduled		= NULL;
	fp_prio_queue_init(&pfp->ready_queue);
}

static void requeue(struct task_struct* t, mrsp_push_domain_t *pfp)
{
...
static struct task_struct* get_high_priority_mrsp_task(struct task_struct** mrsp_array){
	int i;
	for(i = 0; i < LITMUS_MAX_PRIORITY; i++){
		smp_mb();
		if(mrsp_array[i] != NULL)
			return mrsp_array[i];
	}
	return NULL;
}
...
static struct task_struct* mrsp_push_schedule(struct task_struct * prev)
{

	mrsp_push_domain_t* 	pfp = local_pfp;
	struct task_struct*	next;
	int take_from_run_queue;
...
//	struct cpumask res_mask;
	...
	if ((!np || blocks) && (resched || !exists)) {
		...
		if(prev && preempt && !migrate && prev->rt_param.task_params.mrsp_lock_status >0 && prev == prev->rt_param.task_params.mrsp_lock->head){
			TRACE("preempted! at partition: %d at %llu.\n", get_partition(prev) ,litmus_clock());
			...
			prev->rt_param.task_params.cpu = -512;
		...
		smp_mb();
		if(get_high_priority_mrsp_task(mrsp_push_tasks[pfp->cpu]) != NULL){
			struct task_struct* head;
//			spin_lock(&mrsp_push_migrate_lock);
			smp_mb();
			head = get_high_priority_mrsp_task(mrsp_push_tasks[pfp->cpu]);

			if(head != NULL){
				BUG_ON(head->rt_param.task_params.saved_cpu != pfp->cpu);
				BUG_ON(head->rt_param.task_params.mrsp_lock_status == 0);
				BUG_ON(head->rt_param.task_params.mrsp_lock->head != head);
				BUG_ON(atomic_read(&head->rt_param.task_params.mrsp_lock->lock_holder_preempted) != 1);

				if(peak_next == NULL || head->rt_param.task_params.mrsp_lock->prio_per_cpu[pfp->cpu] < get_priority(peak_next)){
					next = head;

					smp_mb();
					mrsp_push_tasks[pfp->cpu][head->rt_param.task_params.mrsp_lock->prio_per_cpu[pfp->cpu]] = NULL;

					BUG_ON(next != head);
					next->rt_param.task_params.cpu = pfp->cpu;
					next->rt_param.task_params.priority = next->rt_param.task_params.mrsp_lock->prio_per_cpu[pfp->cpu];
					smp_wmb();
					...
					TRACE(" resumed on its own processor: %d with priority: %d.\n", get_partition(next), get_priority(next));
				}
			}

//			spin_unlock(&mrsp_push_migrate_lock);
		}

		if(next == NULL){
			smp_rmb();
			if(peak_next != NULL && peak_next->rt_param.task_params.enable_help == 1 && peak_next->rt_param.task_params.mrsp_lock_status > 0 && peak_next != peak_next->rt_param.task_params.mrsp_lock->head){
				//spin_lock(&mrsp_migrate_lock);

				smp_rmb();
				if(atomic_read(&peak_next->rt_param.task_params.mrsp_lock->lock_holder_preempted) == 1){
					struct task_struct *head = peak_next->rt_param.task_params.mrsp_lock->head;

					smp_mb();
					mrsp_push_tasks[head->rt_param.task_params.saved_cpu][head->rt_param.task_params.mrsp_lock->prio_per_cpu[head->rt_param.task_params.saved_cpu]] = NULL;

					smp_wmb();
					atomic_set(&head->rt_param.task_params.mrsp_lock->lock_holder_preempted, 0);
					peak_next->rt_param.task_params.is_helping = 1;
					peak_next->rt_param.task_params.mrsp_lock->helper = peak_next;

					head->rt_param.task_params.priority = head->rt_param.task_params.mrsp_lock->prio_per_cpu[pfp->cpu] -1;
					head->rt_param.task_params.cpu = pfp->cpu;
					next = head;
					atomic_inc(&next->rt_param.task_params.mrsp_lock->resume_by_help);
					atomic_inc(&next->rt_param.task_params.mrsp_lock->resume_by_pull);
					TRACE(" head migrate to %d with priority: %d.\n", get_partition(next), get_priority(next));
				}

				//spin_unlock(&mrsp_migrate_lock);
			}
		...
		if(prev != NULL && next != NULL){
			if(prev->rt_param.task_params.is_helping){
				if(next == prev || next == prev->rt_param.task_params.mrsp_lock->head){
...
static void mrsp_push_finish_switch(struct task_struct *prev)
{

	mrsp_push_domain_t *to;
	struct task_struct *helper;
	unsigned long	flag;
	int failed = 1;

	if (get_partition(prev) == -512) {
//		printk(KERN_EMERG "PREEMPTED at %d Born at %d!.\n", smp_processor_id(), prev->rt_param.task_params.saved_cpu);
		...
		if(helper != NULL && get_partition(helper) != get_partition(prev) && helper->rt_param.task_params.enable_help){
			to = task_pfp(helper);
...
//				printk(KERN_EMERG "we will run after migration F %d to %d.\n", smp_processor_id(), to->cpu);
				atomic_inc(&prev->rt_param.task_params.mrsp_lock->resume_by_help);
				atomic_inc(&prev->rt_param.task_params.mrsp_lock->resume_by_push);
				helper->rt_param.task_params.is_helping = 1;
				helper->rt_param.task_params.mrsp_lock->helper = helper;
				failed = 0;
				preempt(to);
			}else{
//				printk(KERN_EMERG "sorry, we cannot run after migration F %d T %d.\n", smp_processor_id(), to->cpu);
		...
		if(failed){
			mrsp_push_domain_t *pfp;
//			printk(KERN_EMERG "no helping, task goes into preemption list.\n");
			pfp = remote_pfp(prev->rt_param.task_params.saved_cpu);
			//spin_lock(&mrsp_push_migrate_lock);
			raw_spin_lock(&pfp->slock);
			smp_mb();
			mrsp_push_tasks[prev->rt_param.task_params.saved_cpu][prev->rt_param.task_params.mrsp_lock->prio_per_cpu[prev->rt_param.task_params.saved_cpu]] = prev;
			smp_wmb();
		...
		smp_rmb();
		if(prev->rt_param.task_params.mrsp_lock_status >0 && prev == prev->rt_param.task_params.mrsp_lock->head){
			return;
		}
 ...
 */
static void mrsp_push_task_new(struct task_struct * t, int on_rq, int is_scheduled)
{

	mrsp_push_domain_t* 	pfp = task_pfp(t);
	unsigned long		flags;
	...
	if (is_scheduled) {
		...
		mrsp_push_preempt_check(pfp);
	}
	raw_spin_unlock_irqrestore(&pfp->slock, flags);
}

static void mrsp_push_task_wake_up(struct task_struct *task)
{

	unsigned long		flags;
	mrsp_push_domain_t*		pfp = task_pfp(task);
	lt_t			now;

	/*TRACE("mrsp_push: mrsp_push_task_wake_up called.");*/
...
static void mrsp_push_task_block(struct task_struct *t)
{
	/*TRACE("mrsp_push: mrsp_push_task_block called.");*/
...
static void mrsp_push_task_exit(struct task_struct * t)
{
	...
	TRACE("mrsp_push: mrsp_push_task_exit called.");
...
static void mrsp_push_dequeue(mrsp_push_domain_t* pfp, struct task_struct* t)
{
...
static void mrsp_push_set_prio_inh(mrsp_push_domain_t* pfp, struct task_struct* t,
...
static int mrsp_push_effective_agent_priority(int prio)
{
...
fmlp_clock(void)
{
	return (lt_t) this_cpu_inc_return(mrsp_push_fmlp_timestamp);
}

int mrsp_push_fmlp_lock(struct litmus_lock* l)
{
...
int mrsp_push_fmlp_unlock(struct litmus_lock* l)
{
...
int mrsp_push_fmlp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct fmlp_semaphore *sem = fmlp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_push_fmlp_unlock(l);

	return 0;
}

void mrsp_push_fmlp_free(struct litmus_lock* lock)
{
	kfree(fmlp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_push_fmlp_lock_ops = {
	.close  = mrsp_push_fmlp_close,
	.lock   = mrsp_push_fmlp_lock,
	.unlock = mrsp_push_fmlp_unlock,
	.deallocate = mrsp_push_fmlp_free,
};
...
static struct litmus_lock* pfp_new_fmlp(void)
{
	...
	sem->litmus_lock.ops = &mrsp_push_fmlp_lock_ops;
...
int mrsp_push_mpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_push_mpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next = NULL;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
...
int mrsp_push_mpcp_open(struct litmus_lock* l, void* config)
{
	struct task_struct *t = current;
	int cpu, local_cpu;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
...
int mrsp_push_mpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_push_mpcp_unlock(l);

	return 0;
}

void mrsp_push_mpcp_free(struct litmus_lock* lock)
{
	kfree(mpcp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_push_mpcp_lock_ops = {
	.close  = mrsp_push_mpcp_close,
	.lock   = mrsp_push_mpcp_lock,
	.open	= mrsp_push_mpcp_open,
	.unlock = mrsp_push_mpcp_unlock,
	.deallocate = mrsp_push_mpcp_free,
};
...
static struct litmus_lock* pfp_new_mpcp(int vspin)
{
	...
	sem->owner   = NULL;
	init_waitqueue_head(&sem->wait);
	sem->litmus_lock.ops = &mrsp_push_mpcp_lock_ops;
...
static void pcp_priority_inheritance(void)
{
	...
	mrsp_push_set_prio_inh(pfp, current, NULL);
	mrsp_push_set_prio_inh(pfp, blocked, NULL);
	mrsp_push_set_prio_inh(pfp, blocker, NULL);
	...
	if (blocker && blocked &&
		...
		mrsp_push_set_prio_inh(pfp, blocker, blocked);
	}
...
int mrsp_push_pcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_push_pcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mrsp_push_pcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mrsp_push_pcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
	...
	if (owner)
		mrsp_push_pcp_unlock(l);

	return 0;
}

void mrsp_push_pcp_free(struct litmus_lock* lock)
{
	kfree(pcp_from_lock(lock));
}


static struct litmus_lock_ops mrsp_push_pcp_lock_ops = {
	.close  = mrsp_push_pcp_close,
	.lock   = mrsp_push_pcp_lock,
	.open	= mrsp_push_pcp_open,
	.unlock = mrsp_push_pcp_unlock,
	.deallocate = mrsp_push_pcp_free,
};
...
static struct litmus_lock* pfp_new_pcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_push_pcp_lock_ops;
...
int mrsp_push_dpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int eprio = mrsp_push_effective_agent_priority(get_priority(t));
	int from  = get_partition(t);
...
int mrsp_push_dpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int err = 0;
...
int mrsp_push_dpcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int cpu, eprio;
	...
	eprio = mrsp_push_effective_agent_priority(get_priority(t));
...
int mrsp_push_dpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int owner = 0;
	...
	if (owner)
		mrsp_push_dpcp_unlock(l);

	return 0;
}

void mrsp_push_dpcp_free(struct litmus_lock* lock)
{
	kfree(dpcp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_push_dpcp_lock_ops = {
	.close  = mrsp_push_dpcp_close,
	.lock   = mrsp_push_dpcp_lock,
	.open	= mrsp_push_dpcp_open,
	.unlock = mrsp_push_dpcp_unlock,
	.deallocate = mrsp_push_dpcp_free,
};
...
static struct litmus_lock* pfp_new_dpcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_push_dpcp_lock_ops;
...
int mrsp_push_dflp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_push_dflp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int err = 0;
...
int mrsp_push_dflp_open(struct litmus_lock* l, void* __user config)
{
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int cpu;
...
int mrsp_push_dflp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int owner = 0;
	...
	if (owner)
		mrsp_push_dflp_unlock(l);

	return 0;
}

void mrsp_push_dflp_free(struct litmus_lock* lock)
{
	kfree(dflp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_push_dflp_lock_ops = {
	.close  = mrsp_push_dflp_close,
	.lock   = mrsp_push_dflp_lock,
	.open	= mrsp_push_dflp_open,
	.unlock = mrsp_push_dflp_unlock,
	.deallocate = mrsp_push_dflp_free,
};
...
static struct litmus_lock* pfp_new_dflp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_push_dflp_lock_ops;
	sem->owner_cpu = NO_CPU;
	sem->owner   = NULL;
	sem->on_cpu  = on_cpu;
	init_waitqueue_head(&sem->wait);
...
int mrsp_push_mrsp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
	...
	while (1){
		...
		smp_rmb();
		if(t->rt_param.task_params.enable_help && atomic_read(&sem->lock_holder_preempted) == 1){
			mrsp_push_domain_t* 	pfp = task_pfp(t);
			raw_spin_lock(&pfp->slock);
			if(fp_prio_peek(&pfp->ready_queue) == NULL){
				raw_spin_lock(&remote_pfp(sem->head->rt_param.task_params.saved_cpu)->slock);
				smp_rmb();
				if(atomic_read(&sem->lock_holder_preempted) == 1){
					struct task_struct *head = sem->head;

					head->rt_param.task_params.priority = head->rt_param.task_params.mrsp_lock->prio_per_cpu[get_partition(t)] -1;
					head->rt_param.task_params.cpu = get_partition(t);
					requeue(head, pfp);

					if(fp_prio_peek(&pfp->ready_queue) == head){
						smp_mb();
						mrsp_push_tasks[head->rt_param.task_params.saved_cpu][head->rt_param.task_params.mrsp_lock->prio_per_cpu[head->rt_param.task_params.saved_cpu]] = NULL;
						smp_wmb();
						atomic_set(&head->rt_param.task_params.mrsp_lock->lock_holder_preempted, 0);
						...
						atomic_inc(&sem->resume_by_pull);
						t->rt_param.task_params.is_helping = 1;
						t->rt_param.task_params.mrsp_lock->helper = t;
						TRACE(" head migrate to %d with priority: %d.\n", get_partition(head), get_priority(head));
					}
					else{
						fp_prio_remove(&pfp->ready_queue, head, get_priority(head));
						BUG_ON(is_queued(head));
						TRACE("the preempted head cannot execute on: %d because there is a high priority task waiting to run.\n", get_partition(head));
					}

				}
				raw_spin_unlock(&remote_pfp(sem->head->rt_param.task_params.saved_cpu)->slock);
			}
			raw_spin_unlock(&pfp->slock);
			schedule();
		}

		if(t != sem->head && get_partition(t) == get_partition(sem->head))
			schedule();
	}
...
int mrsp_push_mrsp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mrsp_semaphore *sem = mrsp_from_lock(l);
	int err = 0;
	...
	mrsp_push_domain_t *next_head_domain;
	...
	if(next_head != NULL){
		...
		if(next_head->rt_param.task_params.preempt_while_waiting == 1){
			if(get_partition(next_head) != get_partition(t)){
//				spin_lock(&mrsp_push_migrate_lock);

				//printk(KERN_EMERG "head preempted on %d. detect at %d!\n", get_partition(next_head), get_partition(t));
				BUG_ON(!is_queued(next_head));

				fp_prio_remove(&next_head_domain->ready_queue, next_head, priority_index(next_head));

				smp_mb();
				mrsp_push_tasks[next_head->rt_param.task_params.saved_cpu][next_head->rt_param.task_params.mrsp_lock->prio_per_cpu[next_head->rt_param.task_params.saved_cpu]] = next_head;
...
//					spin_lock(&mrsp_push_migrate_lock);
					...
					//printk(KERN_EMERG "head preempted on local processor: %d . detect at %d!\n", get_partition(next_head), get_partition(t));
					BUG_ON(!is_queued(next_head));
...
int mrsp_push_mrsp_open(struct litmus_lock* l, void *config)
{
	struct task_struct *t = current;
	if (!is_realtime(t))
		/* we need to know the real-time priority */
...
int mrsp_push_mrsp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mrsp_semaphore *sem = mrsp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_push_mrsp_unlock(l);

	return 0;
}

void mrsp_push_mrsp_free(struct litmus_lock* lock)
{
	printk(KERN_EMERG "Push. During requesting we have: %d preempt_when running, %d preempt_when_waiting, %d resume_by_itself, "
			"%d resume_by_help, %d preempted again, %d resumed by pull, %d resumed by push, %d bug migration\n",
	 ...
	 atomic_read(&mrsp_from_lock(lock)->preempted_again), atomic_read(&mrsp_from_lock(lock)->resume_by_pull),
	 atomic_read(&mrsp_from_lock(lock)->resume_by_push), atomic_read(&mrsp_from_lock(lock)->bug_migration));

	kfree(mrsp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_push_mrsp_lock_ops = {
	.close  = mrsp_push_mrsp_close,
	.lock   = mrsp_push_mrsp_lock,
	.open	= mrsp_push_mrsp_open,
	.unlock = mrsp_push_mrsp_unlock,
	.deallocate = mrsp_push_mrsp_free,
};

static struct litmus_lock* mrsp_push_new_mrsp(int* config)
{
	...
	sem->litmus_lock.ops = &mrsp_push_mrsp_lock_ops;
...
static long mrsp_push_allocate_lock(struct litmus_lock **lock, int type,
				 void* __user config)
{
	...
	TRACE("mrsp_push: mrsp_push_allocate_lock called.");
	...
	case MRSP_SEM:
		...
		*lock = mrsp_push_new_mrsp(prio_per_cpu);
...
static long mrsp_push_admit_task(struct task_struct* tsk)
{
	TRACE("mrsp_push: mrsp_push_admit_task called.");
...
static long mrsp_push_get_domain_proc_info(struct domain_proc_info **ret)
{

	*ret = &pfp_domain_proc_info;

	TRACE("mrsp_push: mrsp_push_get_domain_proc_info called.");
...
static long mrsp_push_activate_plugin(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		mrsp_push_doms[cpu] = remote_pfp(cpu);
		per_cpu(mrsp_push_fmlp_timestamp,cpu) = 0;
	}

#endif


	TRACE("mrsp_push: mrsp_push_active_plugin called.");
...
static long mrsp_push_deactivate_plugin(void)
{
	TRACE("mrsp_push: mrsp_push_deactive_plugin called.");
...
static struct sched_plugin pfp_plugin __cacheline_aligned_in_smp = {
	.plugin_name		= "mrsp_push",
	.task_new		= mrsp_push_task_new,
	.complete_job		= complete_job,
	.task_exit		= mrsp_push_task_exit,
	.schedule		= mrsp_push_schedule,
	.task_wake_up		= mrsp_push_task_wake_up,
	.task_block		= mrsp_push_task_block,
	.admit_task		= mrsp_push_admit_task,
	.activate_plugin	= mrsp_push_activate_plugin,
	.deactivate_plugin	= mrsp_push_deactivate_plugin,
	.get_domain_proc_info	= mrsp_push_get_domain_proc_info,
#ifdef CONFIG_LITMUS_LOCKING
	.allocate_lock		= mrsp_push_allocate_lock,
	.finish_switch		= mrsp_push_finish_switch,
#endif
};


static int __init mrsp_push_pfp(void)
{

	int i,j;
	//struct fp_prio_queue* q;

	TRACE("mrsp_push: mrsp_push_pfp called.");
	...
	spin_lock_init(&mrsp_push_migrate_lock);
	mrsp_push_tasks =  kmalloc(sizeof(struct task_struct**) * num_online_cpus(), GFP_KERNEL);

	for (i = 0; i < num_online_cpus(); i++) {
		mrsp_push_domain_init(remote_pfp(i), i);

		mrsp_push_tasks[i] = kmalloc(sizeof(struct task_struct*) * LITMUS_MAX_PRIORITY, GFP_KERNEL);
		for(j = 0; j < LITMUS_MAX_PRIORITY; j++){
			mrsp_push_tasks[i][j] = NULL;
		}
	}
