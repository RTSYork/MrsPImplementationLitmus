 ...
 *                   than second. Ties are broken by PID. We modified it so that ties are broken by the time of arrivals.
 *                   As this method is always called with the new task (new task to schedule, new task to insert the ready queue)
 *                   as the first, we return 0 if the first is not higher than second.
 *
		   ...
		   struct task_struct* second)
{
	...
	if (get_priority(first_task) < get_priority(second_task))
		...
		/* Break by PID. We first one that arrives win.*/
