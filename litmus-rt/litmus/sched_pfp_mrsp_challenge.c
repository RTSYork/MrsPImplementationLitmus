...
} mrsp_challenge_domain_t;

DEFINE_PER_CPU(mrsp_challenge_domain_t, mrsp_challenge_domains);

mrsp_challenge_domain_t* mrsp_challenge_doms[NR_CPUS];

#define local_pfp		(this_cpu_ptr(&mrsp_challenge_domains))
#define remote_dom(cpu)		(&per_cpu(mrsp_challenge_domains, cpu).domain)
#define remote_pfp(cpu)	(&per_cpu(mrsp_challenge_domains, cpu))
#define task_dom(task)		remote_dom(get_partition(task))
#define task_pfp(task)		remote_pfp(get_partition(task))


#ifdef CONFIG_LITMUS_LOCKING
DEFINE_PER_CPU(uint64_t,mrsp_challenge_fmlp_timestamp);
#endif

/* we assume the lock is being held */
static void preempt(mrsp_challenge_domain_t *pfp)
{
...
static void mrsp_challenge_pfp_release_jobs(rt_domain_t* rt, struct bheap* tasks)
{
	mrsp_challenge_domain_t *pfp = container_of(rt, mrsp_challenge_domain_t, domain);
	unsigned long flags;
...
static void mrsp_challenge_preempt_check(mrsp_challenge_domain_t *pfp)
{
	if (fp_higher_prio(fp_prio_peek(&pfp->ready_queue), pfp->scheduled))
		preempt(pfp);
}

static void mrsp_challenge_domain_init(mrsp_challenge_domain_t* pfp,
			       int cpu)
{
	fp_domain_init(&pfp->domain, NULL, mrsp_challenge_pfp_release_jobs);
	pfp->cpu      		= cpu;
	pfp->scheduled		= NULL;
	fp_prio_queue_init(&pfp->ready_queue);
}

static void requeue(struct task_struct* t, mrsp_challenge_domain_t *pfp)
{
...
static struct task_struct* mrsp_challenge_schedule(struct task_struct * prev)
{

	mrsp_challenge_domain_t* 	pfp = local_pfp;
...
static void mrsp_challenge_finish_switch(struct task_struct *prev)
{

	mrsp_challenge_domain_t *to;
	struct task_struct *helper;
	unsigned long	flag;
	int old_priority ,failed = 1;
		...
		if(failed){
			...
			if(prev->rt_param.task_params.cpu != prev->rt_param.task_params.saved_cpu){
				...
				if (fp_preemption_needed(&pfp->ready_queue, pfp->scheduled)){
					atomic_inc(&prev->rt_param.task_params.mrsp_lock->resume_itself);
					failed_again = 0;
					preempt(pfp);
				}else{
 ...
 */
static void mrsp_challenge_task_new(struct task_struct * t, int on_rq, int is_scheduled)
{

	mrsp_challenge_domain_t* 	pfp = task_pfp(t);
	unsigned long		flags;
	...
	if (is_scheduled) {
		...
		mrsp_challenge_preempt_check(pfp);
	}
	raw_spin_unlock_irqrestore(&pfp->slock, flags);
}

static void mrsp_challenge_task_wake_up(struct task_struct *task)
{

	unsigned long		flags;
	mrsp_challenge_domain_t*		pfp = task_pfp(task);
	lt_t			now;

	/*TRACE("mrsp_challenge: mrsp_challenge_task_wake_up called.");*/
...
static void mrsp_challenge_task_block(struct task_struct *t)
{
	/*TRACE("mrsp_challenge: mrsp_challenge_task_block called.");*/
...
static void mrsp_challenge_task_exit(struct task_struct * t)
{
	...
	TRACE("mrsp_challenge: mrsp_challenge_task_exit called.");
...
static void mrsp_challenge_dequeue(mrsp_challenge_domain_t* pfp, struct task_struct* t)
{
...
static void mrsp_challenge_set_prio_inh(mrsp_challenge_domain_t* pfp, struct task_struct* t,
...
static int mrsp_challenge_effective_agent_priority(int prio)
{
...
fmlp_clock(void)
{
	return (lt_t) this_cpu_inc_return(mrsp_challenge_fmlp_timestamp);
}

int mrsp_challenge_fmlp_lock(struct litmus_lock* l)
{
...
int mrsp_challenge_fmlp_unlock(struct litmus_lock* l)
{
...
int mrsp_challenge_fmlp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct fmlp_semaphore *sem = fmlp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_challenge_fmlp_unlock(l);

	return 0;
}

void mrsp_challenge_fmlp_free(struct litmus_lock* lock)
{
	kfree(fmlp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_challenge_fmlp_lock_ops = {
	.close  = mrsp_challenge_fmlp_close,
	.lock   = mrsp_challenge_fmlp_lock,
	.unlock = mrsp_challenge_fmlp_unlock,
	.deallocate = mrsp_challenge_fmlp_free,
};
...
static struct litmus_lock* pfp_new_fmlp(void)
{
	...
	sem->litmus_lock.ops = &mrsp_challenge_fmlp_lock_ops;
...
int mrsp_challenge_mpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_challenge_mpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next = NULL;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
...
int mrsp_challenge_mpcp_open(struct litmus_lock* l, void* config)
{
	struct task_struct *t = current;
	int cpu, local_cpu;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
...
int mrsp_challenge_mpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_challenge_mpcp_unlock(l);

	return 0;
}

void mrsp_challenge_mpcp_free(struct litmus_lock* lock)
{
	kfree(mpcp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_challenge_mpcp_lock_ops = {
	.close  = mrsp_challenge_mpcp_close,
	.lock   = mrsp_challenge_mpcp_lock,
	.open	= mrsp_challenge_mpcp_open,
	.unlock = mrsp_challenge_mpcp_unlock,
	.deallocate = mrsp_challenge_mpcp_free,
};
...
static struct litmus_lock* pfp_new_mpcp(int vspin)
{
	...
	sem->owner   = NULL;
	init_waitqueue_head(&sem->wait);
	sem->litmus_lock.ops = &mrsp_challenge_mpcp_lock_ops;
...
static void pcp_priority_inheritance(void)
{
	...
	mrsp_challenge_set_prio_inh(pfp, current, NULL);
	mrsp_challenge_set_prio_inh(pfp, blocked, NULL);
	mrsp_challenge_set_prio_inh(pfp, blocker, NULL);
	...
	if (blocker && blocked &&
		...
		mrsp_challenge_set_prio_inh(pfp, blocker, blocked);
	}
...
int mrsp_challenge_pcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_challenge_pcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mrsp_challenge_pcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mrsp_challenge_pcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
	...
	if (owner)
		mrsp_challenge_pcp_unlock(l);

	return 0;
}

void mrsp_challenge_pcp_free(struct litmus_lock* lock)
{
	kfree(pcp_from_lock(lock));
}


static struct litmus_lock_ops mrsp_challenge_pcp_lock_ops = {
	.close  = mrsp_challenge_pcp_close,
	.lock   = mrsp_challenge_pcp_lock,
	.open	= mrsp_challenge_pcp_open,
	.unlock = mrsp_challenge_pcp_unlock,
	.deallocate = mrsp_challenge_pcp_free,
};
...
static struct litmus_lock* pfp_new_pcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_challenge_pcp_lock_ops;
...
int mrsp_challenge_dpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int eprio = mrsp_challenge_effective_agent_priority(get_priority(t));
	int from  = get_partition(t);
...
int mrsp_challenge_dpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int err = 0;
...
int mrsp_challenge_dpcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int cpu, eprio;
	...
	eprio = mrsp_challenge_effective_agent_priority(get_priority(t));
...
int mrsp_challenge_dpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int owner = 0;
	...
	if (owner)
		mrsp_challenge_dpcp_unlock(l);

	return 0;
}

void mrsp_challenge_dpcp_free(struct litmus_lock* lock)
{
	kfree(dpcp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_challenge_dpcp_lock_ops = {
	.close  = mrsp_challenge_dpcp_close,
	.lock   = mrsp_challenge_dpcp_lock,
	.open	= mrsp_challenge_dpcp_open,
	.unlock = mrsp_challenge_dpcp_unlock,
	.deallocate = mrsp_challenge_dpcp_free,
};
...
static struct litmus_lock* pfp_new_dpcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_challenge_dpcp_lock_ops;
...
int mrsp_challenge_dflp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_challenge_dflp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int err = 0;
...
int mrsp_challenge_dflp_open(struct litmus_lock* l, void* __user config)
{
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int cpu;
...
int mrsp_challenge_dflp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int owner = 0;
	...
	if (owner)
		mrsp_challenge_dflp_unlock(l);

	return 0;
}

void mrsp_challenge_dflp_free(struct litmus_lock* lock)
{
	kfree(dflp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_challenge_dflp_lock_ops = {
	.close  = mrsp_challenge_dflp_close,
	.lock   = mrsp_challenge_dflp_lock,
	.open	= mrsp_challenge_dflp_open,
	.unlock = mrsp_challenge_dflp_unlock,
	.deallocate = mrsp_challenge_dflp_free,
};
...
static struct litmus_lock* pfp_new_dflp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_challenge_dflp_lock_ops;
	sem->owner_cpu = NO_CPU;
	sem->owner   = NULL;
	sem->on_cpu  = on_cpu;
	init_waitqueue_head(&sem->wait);
...
int mrsp_challenge_mrsp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
	...
	while (1){
		...
		if(t->rt_param.task_params.enable_help && atomic_read(&sem->lock_holder_preempted) == 1){
			...
			spin_lock_irqsave(&sem->lock, flags);
			raw_spin_lock(&task_pfp(sem->head)->slock);

			smp_rmb();
			if(atomic_read(&sem->lock_holder_preempted) == 1){
				struct task_struct *peak, *head = sem->head;
				int old_cpu, old_priority;

				old_cpu = get_partition(head);
				old_priority = get_priority(head);
				fp_prio_remove(&task_pfp(sem->head)->ready_queue, head, get_priority(head));
				BUG_ON(is_queued(sem->head));
				raw_spin_unlock(&task_pfp(sem->head)->slock);

				raw_spin_lock(&pfp->slock);
				head->rt_param.task_params.priority = get_priority(t) -1;
				...
				if(peak == head){
					...
					//printk(KERN_EMERG" head migrate to %d with priority: %d.\n", get_partition(head), get_priority(head));
				}
				else{
					fp_prio_remove(&pfp->ready_queue, head, get_priority(head));
					head->rt_param.task_params.cpu = old_cpu;
					head->rt_param.task_params.priority = old_priority;
					BUG_ON(is_queued(head));
					TRACE("the preempted head cannot execute on: %d because there is a high priority task waiting to run.\n", get_partition(head));
					//printk(KERN_EMERG "the preempted head cannot execute on: %d because there is a high priority task waiting to run.\n", get_partition(head));
				}
				spin_unlock_irqrestore(&sem->lock, flags);
				schedule();
			}
...
int mrsp_challenge_mrsp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mrsp_semaphore *sem = mrsp_from_lock(l);
	int err = 0;
	...
	mrsp_challenge_domain_t *next_head_domain;
...
int mrsp_challenge_mrsp_open(struct litmus_lock* l, void *config)
{
	struct task_struct *t = current;
	if (!is_realtime(t))
		/* we need to know the real-time priority */
...
int mrsp_challenge_mrsp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mrsp_semaphore *sem = mrsp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_challenge_mrsp_unlock(l);

	return 0;
}

void mrsp_challenge_mrsp_free(struct litmus_lock* lock)
{
	printk(KERN_EMERG "challenge. During requesting we have: %d preempt_when running, %d preempt_when_waiting, %d resume_by_itself, "
...
static struct litmus_lock_ops mrsp_challenge_mrsp_lock_ops = {
	.close  = mrsp_challenge_mrsp_close,
	.lock   = mrsp_challenge_mrsp_lock,
	.open	= mrsp_challenge_mrsp_open,
	.unlock = mrsp_challenge_mrsp_unlock,
	.deallocate = mrsp_challenge_mrsp_free,
};

static struct litmus_lock* mrsp_challenge_new_mrsp(int* config)
{
	...
	sem->litmus_lock.ops = &mrsp_challenge_mrsp_lock_ops;
...
static long mrsp_challenge_allocate_lock(struct litmus_lock **lock, int type,
				 void* __user config)
{
	...
	TRACE("mrsp_challenge: mrsp_challenge_allocate_lock called.");
	...
	case MRSP_SEM:
		...
		*lock = mrsp_challenge_new_mrsp(prio_per_cpu);
...
static long mrsp_challenge_admit_task(struct task_struct* tsk)
{
	TRACE("mrsp_challenge: mrsp_challenge_admit_task called.");
...
static long mrsp_challenge_get_domain_proc_info(struct domain_proc_info **ret)
{

	*ret = &pfp_domain_proc_info;

	TRACE("mrsp_challenge: mrsp_challenge_get_domain_proc_info called.");
...
static long mrsp_challenge_activate_plugin(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		mrsp_challenge_doms[cpu] = remote_pfp(cpu);
		per_cpu(mrsp_challenge_fmlp_timestamp,cpu) = 0;
	}

#endif


	TRACE("mrsp_challenge: mrsp_challenge_active_plugin called.");
...
static long mrsp_challenge_deactivate_plugin(void)
{
	TRACE("mrsp_challenge: mrsp_challenge_deactive_plugin called.");
...
static struct sched_plugin pfp_plugin __cacheline_aligned_in_smp = {
	.plugin_name		= "mrsp_challenge",
	.task_new		= mrsp_challenge_task_new,
	.complete_job		= complete_job,
	.task_exit		= mrsp_challenge_task_exit,
	.schedule		= mrsp_challenge_schedule,
	.task_wake_up		= mrsp_challenge_task_wake_up,
	.task_block		= mrsp_challenge_task_block,
	.admit_task		= mrsp_challenge_admit_task,
	.activate_plugin	= mrsp_challenge_activate_plugin,
	.deactivate_plugin	= mrsp_challenge_deactivate_plugin,
	.get_domain_proc_info	= mrsp_challenge_get_domain_proc_info,
#ifdef CONFIG_LITMUS_LOCKING
	.allocate_lock		= mrsp_challenge_allocate_lock,
	.finish_switch		= mrsp_challenge_finish_switch,
#endif
};


static int __init mrsp_challenge_pfp(void)
{

	int i;
	//struct fp_prio_queue* q;

	TRACE("mrsp_challenge: mrsp_challenge_pfp called.");
	...
	for (i = 0; i < num_online_cpus(); i++) {
		mrsp_challenge_domain_init(remote_pfp(i), i);
	}

	return register_sched_plugin(&pfp_plugin);
}

module_init(mrsp_challenge_pfp);
