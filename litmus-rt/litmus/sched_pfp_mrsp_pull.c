...
DEFINE_PER_CPU(mrsp_pull_domain_t, mrsp_pull_domains);

mrsp_pull_domain_t* mrsp_pull_doms[NR_CPUS];

spinlock_t mrsp_migrate_lock;
struct task_struct*** mrsp_tasks;

#define local_pfp		(this_cpu_ptr(&mrsp_pull_domains))
#define remote_dom(cpu)		(&per_cpu(mrsp_pull_domains, cpu).domain)
#define remote_pfp(cpu)	(&per_cpu(mrsp_pull_domains, cpu))
#define task_dom(task)		remote_dom(get_partition(task))
#define task_pfp(task)		remote_pfp(get_partition(task))


#ifdef CONFIG_LITMUS_LOCKING
DEFINE_PER_CPU(uint64_t,mrsp_pull_fmlp_timestamp);
#endif

/* we assume the lock is being held */
static void preempt(mrsp_pull_domain_t *pfp)
{
...
static void mrsp_pull_pfp_release_jobs(rt_domain_t* rt, struct bheap* tasks)
{
	mrsp_pull_domain_t *pfp = container_of(rt, mrsp_pull_domain_t, domain);
	unsigned long flags;
...
static void mrsp_pull_preempt_check(mrsp_pull_domain_t *pfp)
{
	if (fp_higher_prio(fp_prio_peek(&pfp->ready_queue), pfp->scheduled))
		preempt(pfp);
}

static void mrsp_pull_domain_init(mrsp_pull_domain_t* pfp,
			       int cpu)
{
	fp_domain_init(&pfp->domain, NULL, mrsp_pull_pfp_release_jobs);
	pfp->cpu      		= cpu;
	pfp->scheduled		= NULL;
	fp_prio_queue_init(&pfp->ready_queue);
}

static void requeue(struct task_struct* t, mrsp_pull_domain_t *pfp)
{
...
static struct task_struct* mrsp_pull_schedule(struct task_struct * prev)
{

	mrsp_pull_domain_t* 	pfp = local_pfp;
...
//	struct cpumask res_mask;
	...
	preempt     = !blocks && (migrate || fp_preemption_needed(&pfp->ready_queue, prev) || prev->rt_param.task_params.mrsp_lock_status == 1);
	...
	if ((!np || blocks) && (resched || !exists)) {
		...
		if(prev && preempt && !migrate && prev->rt_param.task_params.mrsp_lock_status >0 && prev == prev->rt_param.task_params.mrsp_lock->head){
			...
			smp_mb();
			mrsp_tasks[prev->rt_param.task_params.saved_cpu][prev->rt_param.task_params.saved_cpu_ceiling] = prev;
		...
		smp_mb();
		if(get_high_priority_mrsp_task(mrsp_tasks[pfp->cpu]) != NULL){
			struct task_struct* head;
			spin_lock(&mrsp_migrate_lock);
			smp_mb();
			if(get_high_priority_mrsp_task(mrsp_tasks[pfp->cpu]) != NULL){
				smp_mb();
				head = get_high_priority_mrsp_task(mrsp_tasks[pfp->cpu]);
				...
				if(head != NULL){
					...
					if(peak_next == NULL || head->rt_param.task_params.saved_cpu_ceiling < get_priority(peak_next)){
						next = head;

						smp_mb();
						mrsp_tasks[pfp->cpu][head->rt_param.task_params.saved_cpu_ceiling] = NULL;

						BUG_ON(next != head);
						next->rt_param.task_params.cpu = pfp->cpu;
						next->rt_param.task_params.priority = next->rt_param.task_params.saved_cpu_ceiling;
						smp_wmb();
		...
		if(next == NULL){
			...
			if(peak_next != NULL && peak_next->rt_param.task_params.enable_help == 1 && peak_next->rt_param.task_params.mrsp_lock_status > 0 && peak_next != peak_next->rt_param.task_params.mrsp_lock->head){
				...
				if(atomic_read(&peak_next->rt_param.task_params.mrsp_lock->lock_holder_preempted) == 1){
					...
					smp_mb();
					mrsp_tasks[head->rt_param.task_params.saved_cpu][head->rt_param.task_params.saved_cpu_ceiling] = NULL;
					...
					if(next->rt_param.task_params.non_preemption_after_migrate){
						next->rt_param.task_params.priority = 0 /*get_priority(peak_next) */;
						next->rt_param.task_params.migrated_exec_time = get_exec_time(next);
						next->rt_param.task_params.current_cpu_ceiling = get_priority(peak_next) ;
					}
					else
		...
		BUG_ON(preempt && next == prev && prev->rt_param.task_params.mrsp_lock_status == 0);
		/* Similarly, if preempt is set, then next may not be NULL,
		 * unless it's a migration. */
		BUG_ON(preempt && !migrate && next == NULL && prev->rt_param.task_params.mrsp_lock_status == 0);
	} else
...
static void mrsp_pull_finish_switch(struct task_struct *prev)
{

	mrsp_pull_domain_t *to;
 ...
 */
static void mrsp_pull_task_new(struct task_struct * t, int on_rq, int is_scheduled)
{

	mrsp_pull_domain_t* 	pfp = task_pfp(t);
	unsigned long		flags;
	...
	if (is_scheduled) {
		...
		mrsp_pull_preempt_check(pfp);
	}
	raw_spin_unlock_irqrestore(&pfp->slock, flags);
}

static void mrsp_pull_task_wake_up(struct task_struct *task)
{

	unsigned long		flags;
	mrsp_pull_domain_t*		pfp = task_pfp(task);
	lt_t			now;

	/*TRACE("mrsp_pull: mrsp_pull_task_wake_up called.");*/
...
static void mrsp_pull_task_block(struct task_struct *t)
{
	/*TRACE("mrsp_pull: mrsp_pull_task_block called.");*/
...
static void mrsp_pull_task_exit(struct task_struct * t)
{
	...
	TRACE("mrsp_pull: mrsp_pull_task_exit called.");
...
static void mrsp_pull_dequeue(mrsp_pull_domain_t* pfp, struct task_struct* t)
{
...
static void mrsp_pull_set_prio_inh(mrsp_pull_domain_t* pfp, struct task_struct* t,
...
static int mrsp_pull_effective_agent_priority(int prio)
{
...
fmlp_clock(void)
{
	return (lt_t) this_cpu_inc_return(mrsp_pull_fmlp_timestamp);
}

int mrsp_pull_fmlp_lock(struct litmus_lock* l)
{
...
int mrsp_pull_fmlp_unlock(struct litmus_lock* l)
{
...
int mrsp_pull_fmlp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct fmlp_semaphore *sem = fmlp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_pull_fmlp_unlock(l);

	return 0;
}

void mrsp_pull_fmlp_free(struct litmus_lock* lock)
{
	kfree(fmlp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_pull_fmlp_lock_ops = {
	.close  = mrsp_pull_fmlp_close,
	.lock   = mrsp_pull_fmlp_lock,
	.unlock = mrsp_pull_fmlp_unlock,
	.deallocate = mrsp_pull_fmlp_free,
};
...
static struct litmus_lock* pfp_new_fmlp(void)
{
	...
	sem->litmus_lock.ops = &mrsp_pull_fmlp_lock_ops;
...
int mrsp_pull_mpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_pull_mpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next = NULL;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
...
int mrsp_pull_mpcp_open(struct litmus_lock* l, void* config)
{
	struct task_struct *t = current;
	int cpu, local_cpu;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
...
int mrsp_pull_mpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_pull_mpcp_unlock(l);

	return 0;
}

void mrsp_pull_mpcp_free(struct litmus_lock* lock)
{
	kfree(mpcp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_pull_mpcp_lock_ops = {
	.close  = mrsp_pull_mpcp_close,
	.lock   = mrsp_pull_mpcp_lock,
	.open	= mrsp_pull_mpcp_open,
	.unlock = mrsp_pull_mpcp_unlock,
	.deallocate = mrsp_pull_mpcp_free,
};
...
static struct litmus_lock* pfp_new_mpcp(int vspin)
{
	...
	sem->owner   = NULL;
	init_waitqueue_head(&sem->wait);
	sem->litmus_lock.ops = &mrsp_pull_mpcp_lock_ops;
...
static void pcp_priority_inheritance(void)
{
	...
	mrsp_pull_set_prio_inh(pfp, current, NULL);
	mrsp_pull_set_prio_inh(pfp, blocked, NULL);
	mrsp_pull_set_prio_inh(pfp, blocker, NULL);
	...
	if (blocker && blocked &&
		...
		mrsp_pull_set_prio_inh(pfp, blocker, blocked);
	}
...
int mrsp_pull_pcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_pull_pcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mrsp_pull_pcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mrsp_pull_pcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
	...
	if (owner)
		mrsp_pull_pcp_unlock(l);

	return 0;
}

void mrsp_pull_pcp_free(struct litmus_lock* lock)
{
	kfree(pcp_from_lock(lock));
}


static struct litmus_lock_ops mrsp_pull_pcp_lock_ops = {
	.close  = mrsp_pull_pcp_close,
	.lock   = mrsp_pull_pcp_lock,
	.open	= mrsp_pull_pcp_open,
	.unlock = mrsp_pull_pcp_unlock,
	.deallocate = mrsp_pull_pcp_free,
};
...
static struct litmus_lock* pfp_new_pcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_pull_pcp_lock_ops;
...
int mrsp_pull_dpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int eprio = mrsp_pull_effective_agent_priority(get_priority(t));
	int from  = get_partition(t);
...
int mrsp_pull_dpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int err = 0;
...
int mrsp_pull_dpcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int cpu, eprio;
	...
	eprio = mrsp_pull_effective_agent_priority(get_priority(t));
...
int mrsp_pull_dpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int owner = 0;
	...
	if (owner)
		mrsp_pull_dpcp_unlock(l);

	return 0;
}

void mrsp_pull_dpcp_free(struct litmus_lock* lock)
{
	kfree(dpcp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_pull_dpcp_lock_ops = {
	.close  = mrsp_pull_dpcp_close,
	.lock   = mrsp_pull_dpcp_lock,
	.open	= mrsp_pull_dpcp_open,
	.unlock = mrsp_pull_dpcp_unlock,
	.deallocate = mrsp_pull_dpcp_free,
};
...
static struct litmus_lock* pfp_new_dpcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_pull_dpcp_lock_ops;
...
int mrsp_pull_dflp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_pull_dflp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int err = 0;
...
int mrsp_pull_dflp_open(struct litmus_lock* l, void* __user config)
{
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int cpu;
...
int mrsp_pull_dflp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int owner = 0;
	...
	if (owner)
		mrsp_pull_dflp_unlock(l);

	return 0;
}

void mrsp_pull_dflp_free(struct litmus_lock* lock)
{
	kfree(dflp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_pull_dflp_lock_ops = {
	.close  = mrsp_pull_dflp_close,
	.lock   = mrsp_pull_dflp_lock,
	.open	= mrsp_pull_dflp_open,
	.unlock = mrsp_pull_dflp_unlock,
	.deallocate = mrsp_pull_dflp_free,
};
...
static struct litmus_lock* pfp_new_dflp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_pull_dflp_lock_ops;
	sem->owner_cpu = NO_CPU;
	sem->owner   = NULL;
	sem->on_cpu  = on_cpu;
	init_waitqueue_head(&sem->wait);
...
int mrsp_pull_mrsp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
	...
	t->rt_param.task_params.migrated_exec_time = -1;
...
int mrsp_pull_mrsp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mrsp_semaphore *sem = mrsp_from_lock(l);
	int err = 0;
	...
	mrsp_pull_domain_t *next_head_domain;
	...
	cancel_non_preemption_budget_timer();
	...
	if(next_head != NULL){
		...
		if(next_head->rt_param.task_params.preempt_while_waiting == 1){
			if(get_partition(next_head) != get_partition(t)){
				...
				smp_mb();
				mrsp_tasks[next_head->rt_param.task_params.saved_cpu][next_head->rt_param.task_params.saved_cpu_ceiling] = next_head;
...
//		int old_cpu = t->rt_param.task_params.cpu;
		...
		t->rt_param.task_params.priority = t->rt_param.task_params.saved_cpu_ceiling;
		preempt_enable();
		schedule();
...
int mrsp_pull_mrsp_open(struct litmus_lock* l, void *config)
{
	struct task_struct *t = current;
	if (!is_realtime(t))
		/* we need to know the real-time priority */
...
int mrsp_pull_mrsp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mrsp_semaphore *sem = mrsp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_pull_mrsp_unlock(l);

	return 0;
}

void mrsp_pull_mrsp_free(struct litmus_lock* lock)
{
	printk(KERN_EMERG "Pull. During requesting we have: %d preempt_when running, %d preempt_when_waiting, %d resume_by_itself,"
...
static struct litmus_lock_ops mrsp_pull_mrsp_lock_ops = {
	.close  = mrsp_pull_mrsp_close,
	.lock   = mrsp_pull_mrsp_lock,
	.open	= mrsp_pull_mrsp_open,
	.unlock = mrsp_pull_mrsp_unlock,
	.deallocate = mrsp_pull_mrsp_free,
};



static struct litmus_lock* mrsp_pull_new_mrsp(int* config)
{
	...
	BUG_ON(!config);
	prio_per_cpu = kmalloc(sizeof(int) * num_possible_cpus(), GFP_KERNEL);
	sem->head   = NULL;
	...
	copy_from_user (prio_per_cpu, config, sizeof(int)* num_possible_cpus());
	...
	//printk(KERN_EMERG "resource ceiling:");
	//for(i=0 ;i<num_online_cpus(); i++){
	//	printk(KERN_EMERG "%d, ", sem->prio_per_cpu[i]);
	//}
	//printk(KERN_EMERG "\n");

	spin_lock_init(&sem->lock);
	sem->litmus_lock.ops = &mrsp_pull_mrsp_lock_ops;

	//printk(KERN_EMERG "number_possible_cpu: %d. \n", num_possible_cpus());
...
static long mrsp_pull_allocate_lock(struct litmus_lock **lock, int type,
				 void* __user config)
{
	...
	TRACE("mrsp_pull: mrsp_pull_allocate_lock called.");
...
//		get_user(prio_per_cpu, (int*) config);
		...
		*lock = mrsp_pull_new_mrsp(prio_per_cpu);
...
static long mrsp_pull_admit_task(struct task_struct* tsk)
{
	TRACE("mrsp_pull: mrsp_pull_admit_task called.");
...
static long mrsp_pull_get_domain_proc_info(struct domain_proc_info **ret)
{

	*ret = &pfp_domain_proc_info;

	TRACE("mrsp_pull: mrsp_pull_get_domain_proc_info called.");
...
static long mrsp_pull_activate_plugin(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		mrsp_pull_doms[cpu] = remote_pfp(cpu);
		per_cpu(mrsp_pull_fmlp_timestamp,cpu) = 0;
	}

#endif


	TRACE("mrsp_pull: mrsp_pull_active_plugin called.");
...
static long mrsp_pull_deactivate_plugin(void)
{
	TRACE("mrsp_pull: mrsp_pull_deactive_plugin called.");
...
static struct sched_plugin pfp_plugin __cacheline_aligned_in_smp = {
	.plugin_name		= "mrsp_pull",
	.task_new		= mrsp_pull_task_new,
	.complete_job		= complete_job,
	.task_exit		= mrsp_pull_task_exit,
	.schedule		= mrsp_pull_schedule,
	.task_wake_up		= mrsp_pull_task_wake_up,
	.task_block		= mrsp_pull_task_block,
	.admit_task		= mrsp_pull_admit_task,
	.activate_plugin	= mrsp_pull_activate_plugin,
	.deactivate_plugin	= mrsp_pull_deactivate_plugin,
	.get_domain_proc_info	= mrsp_pull_get_domain_proc_info,
#ifdef CONFIG_LITMUS_LOCKING
	.allocate_lock		= mrsp_pull_allocate_lock,
	.finish_switch		= mrsp_pull_finish_switch,
#endif
};

static int __init mrsp_pull_pfp(void)
{

	int i,j;
	//struct fp_prio_queue* q;

	TRACE("mrsp_pull: mrsp_pull_pfp called.");
	...
	mrsp_tasks =  kmalloc(sizeof(struct task_struct**) * num_possible_cpus(), GFP_KERNEL);

	for (i = 0; i < num_possible_cpus(); i++) {
		mrsp_tasks[i] = kmalloc(sizeof(struct task_struct*) * LITMUS_MAX_PRIORITY, GFP_KERNEL);
	...
	for (i = 0; i < num_online_cpus(); i++) {
		mrsp_pull_domain_init(remote_pfp(i), i);
	}

	return register_sched_plugin(&pfp_plugin);
}

module_init(mrsp_pull_pfp);
