...
DEFINE_PER_CPU(mrsp_swap_domain_t, mrsp_swap_domains);

mrsp_swap_domain_t* mrsp_swap_doms[NR_CPUS];

#define local_pfp		(this_cpu_ptr(&mrsp_swap_domains))
#define remote_dom(cpu)		(&per_cpu(mrsp_swap_domains, cpu).domain)
#define remote_pfp(cpu)	(&per_cpu(mrsp_swap_domains, cpu))
#define task_dom(task)		remote_dom(get_partition(task))
#define task_pfp(task)		remote_pfp(get_partition(task))


#ifdef CONFIG_LITMUS_LOCKING
DEFINE_PER_CPU(uint64_t,mrsp_swap_fmlp_timestamp);
#endif

/* we assume the lock is being held */
static void preempt(mrsp_swap_domain_t *pfp)
{
...
static void mrsp_swap_pfp_release_jobs(rt_domain_t* rt, struct bheap* tasks)
{
	mrsp_swap_domain_t *pfp = container_of(rt, mrsp_swap_domain_t, domain);
	unsigned long flags;
...
static void mrsp_swap_preempt_check(mrsp_swap_domain_t *pfp)
{
	if (fp_higher_prio(fp_prio_peek(&pfp->ready_queue), pfp->scheduled))
		preempt(pfp);
}

static void mrsp_swap_domain_init(mrsp_swap_domain_t* pfp,
			       int cpu)
{
	fp_domain_init(&pfp->domain, NULL, mrsp_swap_pfp_release_jobs);
	pfp->cpu      		= cpu;
	pfp->scheduled		= NULL;
	fp_prio_queue_init(&pfp->ready_queue);
}

static void requeue(struct task_struct* t, mrsp_swap_domain_t *pfp)
{
...
int get_nearest_cpu_affinity(struct task_struct *t, struct cpumask res_mask){
	...
	n = cpumask_next(current_cpu, &res_mask);

	//TRACE("n = %d, current_cpu = %d, nb_cpus = %d.\n", n, current_cpu, nb_cpus);

	if( n >= nb_cpus)
		n = cpumask_first(&res_mask);
...
static struct task_struct* mrsp_swap_schedule(struct task_struct * prev)
{

	mrsp_swap_domain_t* 	pfp = local_pfp;
	struct task_struct*	next;

	int out_of_time, sleep, preempt, np, exists, blocks, resched, migrate;

//	int first_cpu, nb_cpus, next_cpu, target_cpu;
//	struct cpumask res_mask;
	...
	if ((!np || blocks) && (resched || !exists)) {
		...
		if(prev && preempt && !migrate && prev->rt_param.task_params.mrsp_lock_status >0 && prev == prev->rt_param.task_params.mrsp_lock->head){
			smp_wmb();
			atomic_set(&prev->rt_param.task_params.mrsp_lock->lock_holder_preempted, 1);
			atomic_inc(&prev->rt_param.task_params.mrsp_lock->preempted_running);
			if(prev->rt_param.task_params.saved_cpu != prev->rt_param.task_params.cpu)
					atomic_inc(&prev->rt_param.task_params.mrsp_lock->preempted_again);
			TRACE("preempted! partition: %d.\n", get_partition(prev));
		}

		smp_rmb();
		if(next != NULL && next->rt_param.task_params.mrsp_lock_status >0 && next == next->rt_param.task_params.mrsp_lock->head &&
				atomic_read(&next->rt_param.task_params.mrsp_lock->lock_holder_preempted) == 1){
			smp_wmb();
			atomic_set(&next->rt_param.task_params.mrsp_lock->lock_holder_preempted, 0);
			if(pfp->cpu == next->rt_param.task_params.saved_cpu)
				atomic_inc(&next->rt_param.task_params.mrsp_lock->resume_itself);
			TRACE("rescheduled with mrsp lock at %d.\n", get_partition(next));
		}

		smp_rmb();
		if(prev && preempt && !migrate && prev->rt_param.task_params.mrsp_lock_status >0 && prev != prev->rt_param.task_params.mrsp_lock->head){
			TRACE("we are preempted while waiting for a lock. partition: %d.\n", get_partition(prev));
			smp_wmb();
			prev->rt_param.task_params.preempt_while_waiting = 1;
		}

		smp_rmb();
		if(next != NULL && next->rt_param.task_params.preempt_while_waiting == 1){
			TRACE("rescheduled when waiting a lock: %d.\n", get_partition(next));
			smp_wmb();
			next->rt_param.task_params.preempt_while_waiting = 0;
		}

		smp_rmb();
		if(prev->rt_param.task_params.is_helping == 1 && migrate){

			BUG_ON(prev == prev->rt_param.task_params.mrsp_lock->head);
			BUG_ON(atomic_read(&prev->rt_param.task_params.mrsp_lock->lock_holder_preempted) != 0);
			BUG_ON(prev->rt_param.task_params.mrsp_lock_status != 1);

			smp_rmb();
			if(next != prev->rt_param.task_params.mrsp_lock->head &&
					get_partition(prev->rt_param.task_params.mrsp_lock->head) == pfp->cpu &&
					is_queued(prev->rt_param.task_params.mrsp_lock->head) ){
				smp_wmb();
				atomic_set(&prev->rt_param.task_params.mrsp_lock->lock_holder_preempted, 1);
				atomic_inc(&prev->rt_param.task_params.mrsp_lock->preempted_running);
				atomic_inc(&prev->rt_param.task_params.mrsp_lock->bug_migration);
			}

			smp_wmb();
			prev->rt_param.task_params.is_helping = 0;
		}
...
static void mrsp_swap_finish_switch(struct task_struct *prev)
{

	mrsp_swap_domain_t *to;
	...
	if (is_realtime(prev) &&
		...
		smp_rmb();
		if(prev->rt_param.task_params.is_migrate_manually == 1){
			return;
		}

		TRACE( "needs to migrate from P%d to P%d. priority = %d. \n", smp_processor_id(), get_partition(prev), get_priority(prev));
		...
		if(is_queued(prev)){
			BUG_ON(prev->rt_param.task_params.mrsp_lock_status != 1);
			printk(KERN_EMERG "BAD thing may happened.\n");
			return;
		}
		requeue(prev, to);
		if (fp_preemption_needed(&to->ready_queue, to->scheduled)){
			struct task_struct *t = fp_prio_peek(&to->ready_queue);
			if(t != prev && prev->rt_param.task_params.mrsp_lock_status >0 ){
				BUG_ON(prev == prev->rt_param.task_params.mrsp_lock->head);
				TRACE("we are preempted after migrate helping. partition: %d.\n", get_partition(prev));
				smp_wmb();
				prev->rt_param.task_params.preempt_while_waiting = 1;
			}
			preempt(to);
			...
			if(prev->rt_param.task_params.mrsp_lock_status >0 ){
				BUG_ON(prev == prev->rt_param.task_params.mrsp_lock->head);
				TRACE("we are preempted after migrate helping. partition: %d.\n", get_partition(prev));
				smp_wmb();
				prev->rt_param.task_params.preempt_while_waiting = 1;
 ...
 */
static void mrsp_swap_task_new(struct task_struct * t, int on_rq, int is_scheduled)
{

	mrsp_swap_domain_t* 	pfp = task_pfp(t);
	unsigned long		flags;
	...
	if (is_scheduled) {
		...
		mrsp_swap_preempt_check(pfp);
	}
	raw_spin_unlock_irqrestore(&pfp->slock, flags);
}

static void mrsp_swap_task_wake_up(struct task_struct *task)
{

	unsigned long		flags;
	mrsp_swap_domain_t*		pfp = task_pfp(task);
	lt_t			now;

	/*TRACE("mrsp_swap: mrsp_swap_task_wake_up called.");*/
...
static void mrsp_swap_task_block(struct task_struct *t)
{
	/*TRACE("mrsp_swap: mrsp_swap_task_block called.");*/
...
static void mrsp_swap_task_exit(struct task_struct * t)
{
	...
	TRACE("mrsp_swap: mrsp_swap_task_exit called.");
...
static void mrsp_swap_dequeue(mrsp_swap_domain_t* pfp, struct task_struct* t)
{
...
static void mrsp_swap_set_prio_inh(mrsp_swap_domain_t* pfp, struct task_struct* t,
...
static int mrsp_swap_effective_agent_priority(int prio)
{
...
fmlp_clock(void)
{
	return (lt_t) this_cpu_inc_return(mrsp_swap_fmlp_timestamp);
}

int mrsp_swap_fmlp_lock(struct litmus_lock* l)
{
...
int mrsp_swap_fmlp_unlock(struct litmus_lock* l)
{
...
int mrsp_swap_fmlp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct fmlp_semaphore *sem = fmlp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_swap_fmlp_unlock(l);

	return 0;
}

void mrsp_swap_fmlp_free(struct litmus_lock* lock)
{
	kfree(fmlp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_swap_fmlp_lock_ops = {
	.close  = mrsp_swap_fmlp_close,
	.lock   = mrsp_swap_fmlp_lock,
	.unlock = mrsp_swap_fmlp_unlock,
	.deallocate = mrsp_swap_fmlp_free,
};
...
static struct litmus_lock* pfp_new_fmlp(void)
{
	...
	sem->litmus_lock.ops = &mrsp_swap_fmlp_lock_ops;
...
int mrsp_swap_mpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_swap_mpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next = NULL;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
...
int mrsp_swap_mpcp_open(struct litmus_lock* l, void* config)
{
	struct task_struct *t = current;
	int cpu, local_cpu;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
...
int mrsp_swap_mpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_swap_mpcp_unlock(l);

	return 0;
}

void mrsp_swap_mpcp_free(struct litmus_lock* lock)
{
	kfree(mpcp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_swap_mpcp_lock_ops = {
	.close  = mrsp_swap_mpcp_close,
	.lock   = mrsp_swap_mpcp_lock,
	.open	= mrsp_swap_mpcp_open,
	.unlock = mrsp_swap_mpcp_unlock,
	.deallocate = mrsp_swap_mpcp_free,
};
...
static struct litmus_lock* pfp_new_mpcp(int vspin)
{
	...
	sem->owner   = NULL;
	init_waitqueue_head(&sem->wait);
	sem->litmus_lock.ops = &mrsp_swap_mpcp_lock_ops;
...
static void pcp_priority_inheritance(void)
{
	...
	mrsp_swap_set_prio_inh(pfp, current, NULL);
	mrsp_swap_set_prio_inh(pfp, blocked, NULL);
	mrsp_swap_set_prio_inh(pfp, blocker, NULL);
	...
	if (blocker && blocked &&
		...
		mrsp_swap_set_prio_inh(pfp, blocker, blocked);
	}
...
int mrsp_swap_pcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_swap_pcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mrsp_swap_pcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mrsp_swap_pcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
	...
	if (owner)
		mrsp_swap_pcp_unlock(l);

	return 0;
}

void mrsp_swap_pcp_free(struct litmus_lock* lock)
{
	kfree(pcp_from_lock(lock));
}


static struct litmus_lock_ops mrsp_swap_pcp_lock_ops = {
	.close  = mrsp_swap_pcp_close,
	.lock   = mrsp_swap_pcp_lock,
	.open	= mrsp_swap_pcp_open,
	.unlock = mrsp_swap_pcp_unlock,
	.deallocate = mrsp_swap_pcp_free,
};
...
static struct litmus_lock* pfp_new_pcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_swap_pcp_lock_ops;
...
int mrsp_swap_dpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int eprio = mrsp_swap_effective_agent_priority(get_priority(t));
	int from  = get_partition(t);
...
int mrsp_swap_dpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int err = 0;
...
int mrsp_swap_dpcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int cpu, eprio;
	...
	eprio = mrsp_swap_effective_agent_priority(get_priority(t));
...
int mrsp_swap_dpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int owner = 0;
	...
	if (owner)
		mrsp_swap_dpcp_unlock(l);

	return 0;
}

void mrsp_swap_dpcp_free(struct litmus_lock* lock)
{
	kfree(dpcp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_swap_dpcp_lock_ops = {
	.close  = mrsp_swap_dpcp_close,
	.lock   = mrsp_swap_dpcp_lock,
	.open	= mrsp_swap_dpcp_open,
	.unlock = mrsp_swap_dpcp_unlock,
	.deallocate = mrsp_swap_dpcp_free,
};
...
static struct litmus_lock* pfp_new_dpcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_swap_dpcp_lock_ops;
...
int mrsp_swap_dflp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mrsp_swap_dflp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int err = 0;
...
int mrsp_swap_dflp_open(struct litmus_lock* l, void* __user config)
{
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int cpu;
...
int mrsp_swap_dflp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int owner = 0;
	...
	if (owner)
		mrsp_swap_dflp_unlock(l);

	return 0;
}

void mrsp_swap_dflp_free(struct litmus_lock* lock)
{
	kfree(dflp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_swap_dflp_lock_ops = {
	.close  = mrsp_swap_dflp_close,
	.lock   = mrsp_swap_dflp_lock,
	.open	= mrsp_swap_dflp_open,
	.unlock = mrsp_swap_dflp_unlock,
	.deallocate = mrsp_swap_dflp_free,
};
...
static struct litmus_lock* pfp_new_dflp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mrsp_swap_dflp_lock_ops;
	sem->owner_cpu = NO_CPU;
	sem->owner   = NULL;
	sem->on_cpu  = on_cpu;
	init_waitqueue_head(&sem->wait);
...
/* Migrate a single helper task */
static void mrsp_swap_helper_migrate_to(int target_cpu, struct mrsp_semaphore* sem, int priority_boost)
{
	struct task_struct* t = current;
	mrsp_swap_domain_t *from;

	BUG_ON(get_partition(t) == target_cpu);
	...
	smp_wmb();
	t->rt_param.task_params.is_migrate_manually = 0;

	/* switch partitions. */
	tsk_rt(t)->task_params.cpu = target_cpu;
	/* If the helper task is moved back to its partition, do not raise its priority. */
	if(priority_boost)
		tsk_rt(t)->task_params.priority = sem->prio_per_cpu[target_cpu] - 1;
	else
		tsk_rt(t)->task_params.priority = sem->prio_per_cpu[target_cpu];
	raw_spin_unlock(&from->slock);
	...
	/* we are now on the target processor */
	preempt_disable();
	//printk(KERN_EMERG "helper is gone F %d to %d.!\n",from->cpu ,smp_processor_id());
}

/*
 * Called when the lock holder is preempted and a waiting task is be able to help.
 * This method will be called by all waiting tasks but finally only one helper task will perform the swap.
 */
static void mrsp_swap_mrsp_swap_migration(struct task_struct* helper, struct mrsp_semaphore* sem)
{
	unsigned long flags;
	mrsp_swap_domain_t *owner_domain;
	mrsp_swap_domain_t *waiter_domain;
	int owner_cpu;

	/* We detected that the holder task is preempted.*/
	smp_rmb();
	if(atomic_read(&sem->lock_holder_preempted) == 1){
		spin_lock_irqsave(&sem->lock, flags);
		/* We verify that the holder task is still holding the lock.*/
		smp_rmb();
		if(atomic_read(&sem->lock_holder_preempted) == 1 && sem->head != NULL && sem->head != helper){
			owner_domain = task_pfp(sem->head);

			raw_spin_lock(&owner_domain->slock);
			/* We confirm that the lock holder task is indeed preempted and we are now initiate the helping (swapping) routine. */
			smp_rmb();
			if(atomic_read(&sem->lock_holder_preempted) == 1){
				preempt_disable();

				smp_wmb();
				sem->head->rt_param.task_params.is_migrate_manually = 1;

				smp_wmb();
				atomic_set(&sem->lock_holder_preempted, 0);		// Cancel the preemption flag.
				atomic_inc(&sem->resume_by_help);

				//printk(KERN_EMERG "the holder needs our help. F: %d, T: %d!\n", get_partition(sem->head), get_partition(helper));

				/* migrate the lock holder task to the helper's CPU.*/
				BUG_ON(!is_queued(sem->head));
				fp_prio_remove(&owner_domain->ready_queue, sem->head, priority_index(sem->head)); 	// remove the holder from its domain.


				raw_spin_unlock(&owner_domain->slock);


				waiter_domain = task_pfp(helper);
				raw_spin_lock(&waiter_domain->slock);
				smp_wmb();
				helper->rt_param.task_params.is_helping = 1;
				owner_cpu = sem->head->rt_param.task_params.cpu;
				sem->head->rt_param.task_params.cpu = get_partition(helper);
				/* Raise its priority so that it will not be preempted and stuck when it tries to migrate back.*/
				sem->head->rt_param.task_params.priority = sem->prio_per_cpu[get_partition(helper)] - 1;
				fp_prio_add(&waiter_domain->ready_queue, sem->head, get_priority(sem->head)); 	// add the lock holder to the helper's domain.

				raw_spin_unlock(&waiter_domain->slock);
				spin_unlock_irqrestore(&sem->lock, flags);

				/* Migrate the helper to the holder's CPU. IMPORTANT: preemption remains disable at this point. */
				//printk(KERN_EMERG "helper will go F %d to %d.!\n",smp_processor_id() ,owner_cpu);
				mrsp_swap_helper_migrate_to(owner_cpu, sem, 1);		// Again, raise the helper's priority so that it will not be preempted and stuck when it needs to migrate back.
				preempt_enable();
...
int mrsp_swap_mrsp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
	...
	/* raise the priority to the ceiling and save its original priority and CPU. */
	...
	/* Get a ticket and set the task's status to "waiting for the lock" */
	spin_lock_irqsave(&sem->lock, flags);
	smp_rmb();
	...
	TRACE("I got ticket: %d, partition %d. \n", ticket, get_partition(t));
	...
	t->rt_param.task_params.mrsp_lock_status = 1;
	spin_unlock_irqrestore(&sem->lock, flags);

	/*Spinning for the lock.*/
	while (1){
		smp_rmb();
		/* When all the helper tasks are migrated back and we have the right ticket. Then we can obtain the lock. And we will set our status to "holding the lock". */
		if (atomic_read(&sem->owner_ticket) == ticket){

			//spin_lock_irqsave(&sem->lock, flags);
			//BUG_ON(sem->head != t);
			//TRACE("ticket: %d, owner: %d, partition %d. \n", ticket, atomic_read(&sem->owner_ticket),get_partition(t));

			t->rt_param.task_params.mrsp_lock_status = 2;
			//spin_unlock_irqrestore(&sem->lock, flags);
			break;
		}

		/*If the holder is preempted while spinning, we will see whehter we can help. */
		if(t->rt_param.task_params.enable_help){
			mrsp_swap_mrsp_swap_migration(t, sem);

			preempt_disable();
			smp_rmb();
			if(t == sem->helper && smp_processor_id() != get_partition(t)){
				//printk(KERN_EMERG "helper will come.\n");
				preempt_enable();
				schedule();
				//printk(KERN_EMERG "helper is coming.\n");
			}
			else
				preempt_enable();
		}
	...
	BUG_ON(sem->head != t);
	TRACE_TASK(t, "we got the lock !!: %d \n", get_partition(t));
...
int mrsp_swap_mrsp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mrsp_semaphore *sem = mrsp_from_lock(l);
	int err = 0;
	unsigned long flags;
	struct task_struct *next_head = NULL;
	mrsp_swap_domain_t *next_head_domain;
	struct task_struct *helper = NULL;
	mrsp_swap_domain_t *helper_domain, *owner_domain;
	...
	/* Increase the ticket so that the next waiting task can obtain the lock and set our status to "has nothing to do with the lock". */
	preempt_disable();
	spin_lock_irqsave(&sem->lock, flags);
	smp_wmb();
	t->rt_param.task_params.is_migrate_manually = 0;

	// wait for the helper to migrate back
	if(t->rt_param.task_params.cpu != t->rt_param.task_params.saved_cpu){
		...
		while(1){
			smp_mb();
			if(helper->rt_param.task_params.cpu == t->rt_param.task_params.saved_cpu)
				break;
			smp_mb();
			helper = helper == NULL? NULL: helper->rt_param.task_params.next;
			if(helper == NULL)
				break;
		}
		if(helper == NULL){
		...
		smp_wmb();
		helper->rt_param.task_params.is_migrate_manually = 1;
		smp_wmb();
		sem->helper = helper;

		helper_domain = task_pfp(helper);
		raw_spin_lock(&helper_domain->slock);

		if(is_queued(helper))
			fp_prio_remove(&helper_domain->ready_queue, helper, priority_index(helper));

		helper->rt_param.task_params.cpu = get_partition(t);
		helper->rt_param.task_params.priority = sem->prio_per_cpu[get_partition(t)];

		raw_spin_unlock(&helper_domain->slock);


		owner_domain = task_pfp(t);
		raw_spin_lock(&owner_domain->slock);
		fp_prio_add(&owner_domain->ready_queue, helper, get_priority(helper)); 	// add the lock holder to the helper's domain.
		raw_spin_unlock(&owner_domain->slock);

		while(1){
			smp_mb();
			if(helper_domain->scheduled != helper)
				break;
		}
		//printk(KERN_EMERG " we confirm helper is coming F %d to %d.\n", t->rt_param.task_params.saved_cpu, get_partition(t));
	}

	smp_rmb();
	BUG_ON(atomic_read(&sem->lock_holder_preempted) == 1);

	next_head = t->rt_param.task_params.next;
	if(next_head != NULL){
		t->rt_param.task_params.next = NULL;
		BUG_ON(next_head == NULL);
		BUG_ON(next_head->rt_param.task_params.mrsp_lock_status != 1);
		BUG_ON(next_head->rt_param.task_params.ticket != atomic_read(&sem->owner_ticket) + 1);

		next_head_domain = task_pfp(next_head);
		raw_spin_lock(&next_head_domain->slock);

		if(next_head->rt_param.task_params.preempt_while_waiting == 1){
			if(next_head != helper){
				//printk(KERN_EMERG "head preempted on %d. detect at %d!\n", get_partition(next_head), get_partition(t));
				BUG_ON(!is_queued(next_head));
				next_head->rt_param.task_params.preempt_while_waiting = 0;
				smp_wmb();
				atomic_set(&sem->lock_holder_preempted, 1);
				atomic_inc(&sem->preempted_waiting);
			}
			else{
				struct task_struct *t = fp_prio_peek(&next_head_domain->ready_queue);
				if(t != next_head ){
					//printk(KERN_EMERG "head preempted on %d. detect at %d!\n", get_partition(next_head), get_partition(t));
					BUG_ON(!is_queued(next_head));
					next_head->rt_param.task_params.preempt_while_waiting = 0;
	...
	t->rt_param.task_params.mrsp_lock_status = 0;

	TRACE( "we release the lock !! %d. owner: %d. \n", get_partition(t), atomic_read(&sem->owner_ticket));
	spin_unlock_irqrestore(&sem->lock, flags);

	/* Migrate back to our own CPU. */
	if(t->rt_param.task_params.cpu != t->rt_param.task_params.saved_cpu){
		//int old_cpu = t->rt_param.task_params.cpu;
		t->rt_param.task_params.cpu = t->rt_param.task_params.saved_cpu;
		t->rt_param.task_params.priority = sem->prio_per_cpu[get_partition(t)];
		preempt_enable();
		schedule();
//		printk(KERN_EMERG "the old owner is back F %d to %d.\n", old_cpu ,get_partition(t));
	}
	else
		preempt_enable();

	/* Restore our original priority and decrease the number of holing locks. */
...
int mrsp_swap_mrsp_open(struct litmus_lock* l, void *config)
{
	struct task_struct *t = current;
	if (!is_realtime(t))
		/* we need to know the real-time priority */
...
int mrsp_swap_mrsp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mrsp_semaphore *sem = mrsp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		mrsp_swap_mrsp_unlock(l);

	return 0;
}

void mrsp_swap_mrsp_free(struct litmus_lock* lock)
{
	printk(KERN_EMERG "Swap. During requesting we have: %d preempt_when running, %d preempt_when_waiting, %d resume_by_itself,"
			" %d resume_by_help, %d preempted again %d, bug migration. \n",
	 atomic_read(&mrsp_from_lock(lock)->preempted_running), atomic_read(&mrsp_from_lock(lock)->preempted_waiting),
	 atomic_read(&mrsp_from_lock(lock)->resume_itself), atomic_read(&mrsp_from_lock(lock)->resume_by_help),
	 atomic_read(&mrsp_from_lock(lock)->preempted_again), atomic_read(&mrsp_from_lock(lock)->bug_migration));

	kfree(mrsp_from_lock(lock));
}

static struct litmus_lock_ops mrsp_swap_mrsp_lock_ops = {
	.close  = mrsp_swap_mrsp_close,
	.lock   = mrsp_swap_mrsp_lock,
	.open	= mrsp_swap_mrsp_open,
	.unlock = mrsp_swap_mrsp_unlock,
	.deallocate = mrsp_swap_mrsp_free,
};

static struct litmus_lock* mrsp_swap_new_mrsp(int* config)
{
	...
	atomic_set(&sem->lock_holder_preempted, 0);
	atomic_set(&sem->helper_migrate_back, 0);

	atomic_set(&sem->preempted_running, 0);
	atomic_set(&sem->preempted_waiting, 0);
	atomic_set(&sem->resume_itself, 0);
	atomic_set(&sem->resume_by_help, 0);
	atomic_set(&sem->preempted_again, 0);
	atomic_set(&sem->resume_by_push, 0);
	atomic_set(&sem->resume_by_pull, 0);
	atomic_set(&sem->bug_migration, 0);

	copy_from_user (prio_per_cpu, config, sizeof(int)* num_online_cpus());
	//printk(KERN_EMERG "num online cpu: %d.\n", num_online_cpus());
	...
	sem->litmus_lock.ops = &mrsp_swap_mrsp_lock_ops;
...
static long mrsp_swap_allocate_lock(struct litmus_lock **lock, int type,
				 void* __user config)
{
	...
	TRACE("mrsp_swap: mrsp_swap_allocate_lock called.");
	...
	case MRSP_SEM:
		...
		*lock = mrsp_swap_new_mrsp(prio_per_cpu);
...
static long mrsp_swap_admit_task(struct task_struct* tsk)
{
	TRACE("mrsp_swap: mrsp_swap_admit_task called.");
...
static long mrsp_swap_get_domain_proc_info(struct domain_proc_info **ret)
{

	*ret = &pfp_domain_proc_info;

	TRACE("mrsp_swap: mrsp_swap_get_domain_proc_info called.");
...
static long mrsp_swap_activate_plugin(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		mrsp_swap_doms[cpu] = remote_pfp(cpu);
		per_cpu(mrsp_swap_fmlp_timestamp,cpu) = 0;
	}

#endif


	TRACE("mrsp_swap: mrsp_swap_active_plugin called.");
...
static long mrsp_swap_deactivate_plugin(void)
{
	TRACE("mrsp_swap: mrsp_swap_deactive_plugin called.");
...
static struct sched_plugin pfp_plugin __cacheline_aligned_in_smp = {
	.plugin_name		= "mrsp_swap",
	.task_new		= mrsp_swap_task_new,
	.complete_job		= complete_job,
	.task_exit		= mrsp_swap_task_exit,
	.schedule		= mrsp_swap_schedule,
	.task_wake_up		= mrsp_swap_task_wake_up,
	.task_block		= mrsp_swap_task_block,
	.admit_task		= mrsp_swap_admit_task,
	.activate_plugin	= mrsp_swap_activate_plugin,
	.deactivate_plugin	= mrsp_swap_deactivate_plugin,
	.get_domain_proc_info	= mrsp_swap_get_domain_proc_info,
#ifdef CONFIG_LITMUS_LOCKING
	.allocate_lock		= mrsp_swap_allocate_lock,
	.finish_switch		= mrsp_swap_finish_switch,
#endif
};


static int __init mrsp_swap_pfp(void)
{

	int i;

	TRACE("mrsp_swap: mrsp_swap_pfp called.");
	...
	for (i = 0; i < num_online_cpus(); i++) {
		mrsp_swap_domain_init(remote_pfp(i), i);
	}
	return register_sched_plugin(&pfp_plugin);
}

module_init(mrsp_swap_pfp);
