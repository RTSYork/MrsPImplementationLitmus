...
//		if(p->rt_param.task_params.mrsp_lock_status > 0 && p->rt_param.task_params.migrated_exec_time >= 0){
//			if(get_priority(p) == 0){
//
//				TRACE("current %llu. migration %llu.\n", get_exec_time(p), p->rt_param.task_params.migrated_exec_time);
//				if(get_exec_time(p) - p->rt_param.task_params.migrated_exec_time >= p->rt_param.task_params.double_migration_cost){
//					TRACE("total %llu.\n", get_exec_time(p) - p->rt_param.task_params.migrated_exec_time);
//					p->rt_param.task_params.priority = p->rt_param.task_params.current_cpu_ceiling;
//					p->rt_param.task_params.migrated_exec_time = -1;
//					litmus_reschedule_local();
//				}
//			}
//			else
//				printk("RACE condition! holder released the lock already.\n");
//
//		}
	}
}
